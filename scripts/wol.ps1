...
.SUBCOMMANDS
  ...
  add-dev  <id[|Name][|MAC][|Secret]> [...]
                                     Add one or more devices to an existing deployment.
  install  [-DeviceId .. -ApiUrl .. [-Secret ..] [-RegisterAws]]
                                     Install the polling agent + scheduled task on this PC
                                     (auto-generates ad-hoc secret if -Secret is omitted).
...
usage: wol.ps1 <subcommand> [args...]
  ...
  add-dev  <id[|Name][|MAC][|Secret]> [...] add devices to an existing deployment
  install  [-DeviceId .. -ApiUrl .. [-Secret ..] [-RegisterAws]]
                                      install the agent + scheduled task (auto-generates secret if omitted)
